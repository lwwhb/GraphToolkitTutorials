...
builder = renderGraph.AddRasterRenderPass<PassData>("Opaque Pass", out var passData))
            {
                passData.node = node;
                passData.cameraColor = resourceData.activeColorTexture;
                passData.cameraDepth = resourceData.activeDepthTexture;
                ...
                builder.SetRenderFunc((PassData data, RasterGraphContext context) =>
                {
                    // 新API：直接使用RasterCommandBuffer，无需手动管理CommandBuffer生命周期
        ...
        private void AddBlitPass(RenderGraph renderGraph, UniversalResourceData resourceData, BlitPassRuntimeNode node)
        {
            ...
            // 第一步：将当前颜色缓冲Blit到临时纹理
            using (var builder = renderGraph.AddRasterRenderPass<PassData>("Blit To Temp", out var passData))
            {
                passData.cameraColor = resourceData.activeColorTexture;
                ...
                builder.SetRenderFunc((PassData data, RasterGraphContext context) =>
                {
                    // 使用Blitter工具类（Unity 6推荐方式）
                    Blitter.BlitTexture(context.cmd, data.cameraColor,
                        new Vector4(1, 1, 0, 0), 0, false);
                });
            }

            // 第二步：将临时纹理（可能经过后处理）Blit回颜色缓冲
            using (var builder = renderGraph.AddRasterRenderPass<PassData>("Blit To Camera", out var passData))
            {
